using DevWiki.Application.DTOs;
...
public class GetArticleBySlugQueryHandler : IRequestHandler<GetArticleBySlugQuery, ApiResponse<ArticleDto>>
{
    private readonly IUnitOfWork _unitOfWork;

    public GetArticleBySlugQueryHandler(IUnitOfWork unitOfWork)
    {
        _unitOfWork = unitOfWork;
    }

    public async Task<ApiResponse<ArticleDto>> Handle(GetArticleBySlugQuery request, CancellationToken cancellationToken)
    {
        var article = await _unitOfWork.Articles.GetBySlugAsync(request.Slug, cancellationToken);
