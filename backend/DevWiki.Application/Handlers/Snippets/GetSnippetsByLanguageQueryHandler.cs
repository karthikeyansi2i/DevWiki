...
public class GetSnippetsByLanguageQueryHandler : IRequestHandler<GetSnippetsByLanguageQuery, ApiResponse<PaginatedResult<CodeSnippetSearchResultDto>>>
{
    private readonly IUnitOfWork _unitOfWork;

    public GetSnippetsByLanguageQueryHandler(IUnitOfWork unitOfWork)
    {
        _unitOfWork = unitOfWork;
    }

    public async Task<ApiResponse<PaginatedResult<CodeSnippetSearchResultDto>>> Handle(
        GetSnippetsByLanguageQuery request, CancellationToken cancellationToken)
    {
        var (items, totalCount) = await _unitOfWork.CodeSnippets.GetByLanguageAsync(
            request.Language, request.Page, request.PageSize, cancellationToken);
